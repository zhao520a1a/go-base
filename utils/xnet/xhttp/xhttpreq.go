...
func DoWriteResponse(w http.ResponseWriter, header http.Header, cookies []*http.Cookie, status int, body io.Reader) {
	...
	if err != nil {
		xlog.Warnf(context.TODO(), "%s white response n:%d err:%s", fun, n, err)
	}
...
func (m *HttpRespJson) WriteResponse(w http.ResponseWriter) {
	...
	if err != nil {
		xlog.Warnf(context.TODO(), "%s json unmarshal err:%s", fun, err)
...
func (m *reqArgs) Int(key string) int {
	...
	if err != nil {
		xlog.Warnf(context.TODO(), "%s parse int v:%s err:%s", fun, v, err)
	}
	return i
...
func (m *reqArgs) Int32(key string) int32 {
	...
	if err != nil {
		xlog.Warnf(context.TODO(), "%s parse int32 v:%s err:%s", fun, v, err)
	}
...
func (m *reqArgs) Int64(key string) int64 {
	...
	if err != nil {
		xlog.Warnf(context.TODO(), "%s parse int64 v:%s err:%s", fun, v, err)
	}
	return i
...
func (m *reqQuery) Get(key string) string {
	...
	if m.q == nil {
		if m.r.URL != nil {
			...
			if err != nil {
				xlog.Warnf(context.TODO(), "%s parse query q:%s err:%s", fun, m.r.URL.RawQuery, err)
			}
		}
		...
		//xlog.Debugf("%s parse query q:%s err:%s", fun, m.r.URL.RawQuery, m.q)
	}
...
func (m *reqCookie) Get(key string) string {
	...
	if err == http.ErrNoCookie {
		...
		xlog.Warnf(context.TODO(), "%s parse cookie key:%s err:%s", fun, key, err)
		return ""
	}
...
func (m *reqBody) Binary() []byte {
	...
	if m.body == nil {
		...
		if err != nil {
			if err != io.EOF {
				xlog.Warnf(context.TODO(), "%s read body %s", fun, err.Error())
			}
			xlog.Warnf(context.TODO(), "%s read body %s", fun, err.Error())
		}
		m.body = body
...
func (m *reqBody) BinaryUnGzip() []byte {
	...
	if m.body == nil {
		...
		if err != nil {
			xlog.Errorf(context.TODO(), "%s unzip body %s", fun, err.Error())
		}
		defer r.Close()

		body, err := ioutil.ReadAll(r)
		if err != nil {
			xlog.Errorf(context.TODO(), "%s read body %s", fun, err.Error())
		}
		m.body = body
...
func (m *reqBody) FormValue(key string) string {
	...
	if err != nil {
		xlog.Errorf(context.TODO(), "%s parsemediatype err:%s", fun, err)
	}
	...
	if ct == "application/x-www-form-urlencoded" {
		...
		if m.r.MultipartForm == nil {
			xlog.Warnf(context.TODO(), "%s multipart/form-data parse nil", fun)
			return ""
		}
...
func HttpRequestWrapper(fac FactoryHandleRequest) func(http.ResponseWriter, *http.Request, httprouter.Params) {
	...
	return func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		if err != nil {
			xlog.Warnf(context.TODO(), "%s new request err:%s", fun, err)
...
func HttpRequestJsonBodyWrapper(fac FactoryHandleRequest) func(http.ResponseWriter, *http.Request, httprouter.Params) {
	...
	return func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		if err != nil {
			xlog.Warnf(context.TODO(), "%s body json err:%s", fun, err)
...
func validStatusCode(code int) int {
	...
	xlog.Warnf(context.TODO(), "invalid code %d", code)
